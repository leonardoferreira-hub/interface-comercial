...
// Registro B3 para Debênture Pública: Tabela progressiva × prazo POR SÉRIE
// Aplicável a: DEB oferta pública (CVM 160)
// Faixas:
// - Até 100MM: 0.0020%
// - 100MM a 500MM: 0.0015%
// - 500MM a 1BI: 0.0010%
// - Acima 1BI: 0.0005%
...
const FAIXAS: Faixa[] = [
  { limite: 100_000_000, aliquota: 0.0020 / 100 },        // Até 100MM
  { limite: 500_000_000, aliquota: 0.0015 / 100 },        // 100MM a 500MM
  { limite: 1_000_000_000, aliquota: 0.0010 / 100 },      // 500MM a 1BI
  { limite: Infinity, aliquota: 0.0005 / 100 },            // Acima de 1BI
];
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`📊 [registrob3_debpub] series=${JSON.stringify(series)}, volume=${volume}, prazoGeral=${prazoGeral}`);
    ...
    console.log(`✅ [registrob3_debpub] Calculado: R$ ${valorTotalGeral.toFixed(2)} (${series.length} séries)`);

    return new Response(
      JSON.stringify({
        success: true,
        data: {
          nome: "Registro B3 - Debênture Pública",
          papel: "Registro B3",
          valor_total: valorTotalGeral,
          tipo_custo: "variavel",
          periodicidade: "upfront",
          formula: "Tabela progressiva × prazo (anos) por série",
          detalhes: {
    ...
    console.error("💥 [registrob3_debpub] Exceção:", error);
    return new Response(
